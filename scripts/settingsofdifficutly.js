Events.on(EventType.ClientLoadEvent, () => {
    ...
    Vars.ui.menufrag.addButton("@cruxahh-difficulty-button",Icon.logic,() => {
        ...
        let buttonLaun = CSprompt.cont.button(Core.bundle.get("cruxahh-difficulty-launchpad")+(Core.settings.getBool("cruxahh-laun")?stringDisabled:stringEnabled),()=>{
            ...
            if(buf)Blocks.launchPad.buildVisibility=BuildVisibility.campaignOnly;
            else Blocks.launchPad.buildVisibility=BuildVisibility.editorOnly;
            Core.settings.put("cruxahh-laun",!buf);
            buttonLaun.setText(Core.bundle.get("cruxahh-difficulty-launchpad")+(buf?stringEnabled:stringDisabled));
        });
        buttonLaun.size(600,75).row();
        buttonLaun=buttonLaun.get();
        let buttonWave = CSprompt.cont.button(Core.bundle.get("cruxahh-difficulty-wavesim")+(Core.settings.getBool("cruxahh-wave")?stringDisabled:stringEnabled),()=>{
            ...
            buttonWave.setText(Core.bundle.get("cruxahh-difficulty-wavesim")+(buf?stringEnabled:stringDisabled));
        });
        buttonWave.size(600,75).row();
        buttonWave=buttonWave.get();
